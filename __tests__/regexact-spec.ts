...
function testingProcedure ({ regexp, input, isExecNull, group, execLen, groupText, groupIndex } : { regexp: RegExp, input: string, isExecNull: boolean, group: number, execLen: number, groupText: string, groupIndex: number}) {
  const regex = new RegExp(regexp);
  const regexExec = regex.exec(input);
  expect(regexExec === null).toBe(isExecNull);

  const regexact = new RegExact(regexp);
  const regexactExec = regexact.exec(input);
